...
* Протокол системного журнала позволяет собирать и сохранять логи с сетевого оборудования
* Подразумевается, что существует специальный сервер для хранения логов, куда клиенты отправляют настроенные логи
* Стандарт. Протокол не зависит от производителя и работает на любом оборудовании(любой ос)
* Порт udp 514
* Служба журналирования представляет 3 основные возможности:
  1. Сбор информации в журнал для мониторинга и отладки
  2. Выбор типа информации, сбор которой будет осуществляться(Информация какого уровня будет собрана и сохранена). Также есть возможность тонкой настройки сбора и хранения информации для различных уровней
  3. Определение получателей собранных сообщений syslog (буфер ведения журнала, порт консоли, линия терминала, сервер Syslog)
...
* facility - часть оборудования, с которым произошло событие
* severity - уровень важности события
* mnemonic - сокращенное описание события
* description - полное описание события

Формат лог-сообщений можно поменять, добавив в начало текущую временную метку. Если время не было настроено, либо настройка была неправильной, то время будут отображаться не верно, что может затруднять процесс нахождения и исправления ошибок. 
...
### Пример (настройка временной метки)
![alt text](image-21.png)

### Просмотр журнала сообщений

**По умолчанию все логи выводятся на консольную линию**  
**По умолчанию собираются логи с 7 по 0 уровень**
...
> > сохранение логов в буфер оперативной памяти
> > несмотря на то, что мы настраивам логирование в буфер, логи также будут выводится на консоли терминала.

**Просмотр журнала сообщений**
> R# show logging

### Пример (команда show logging)
...
> > вместо level указывается номер уровня, начиная с которого нужно отправлять сообщения на сервер(т.е этот номер и всё что меньше)
> > нужно сделать, чтобы отправлять не все сообщения, а только некоторых типов

**Настройка интерфейса источника** (Опционально)
> R(config)# logging source-interface *interface-type interface-number*
> > информация об интерфейсе(его ip адрес) источника будет прикреплена к лог сообщению и отправлена на syslog сервер.
> > Это не влияет на то, с какого интерфейса будут отправляться лог сообщения.

### Пример(настройка syslog сервера)

![alt text](image-23.png)

![alt text](image-24.png)
скриншот с сообщениям на syslog сервере
  
На выделенном сервере должно быть настроено специальное ПО для Syslog

### Команда show logging
Позволяет просмотреть все логи, сохраненные на устройстве. Можно использовать команду include для удобной фильтрации
![alt text](image-25.png)

Также сообщения можно фильтровать по дате и времени
